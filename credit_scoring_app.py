...
with col2:
    Payment_of_Min_Amount = st.selectbox(
        label="Payment_of_Min_Amount",
        options=encoder_Payment_of_Min_Amount.classes_,
        index=1,
    )
...
with col3:
    Num_of_Delayed_Payment = int(
        st.number_input(label="Num_of_Delayed_Payment", value=7)
    )
    data["Num_of_Delayed_Payment"] = Num_of_Delayed_Payment

with col4:
    Changed_Credit_Limit = float(
        st.number_input(label="Changed_Credit_Limit", value=11.27)
    )
...
with col3:
    Monthly_Inhand_Salary = float(
        st.number_input(label="Monthly_Inhand_Salary", value=1824.8)
    )
...
with col1:
    Amount_invested_monthly = float(
        st.number_input(label="Amount_invested_monthly", value=236.64)
    )
    data["Amount_invested_monthly"] = Amount_invested_monthly

with col2:
    Total_EMI_per_month = float(
        st.number_input(label="Total_EMI_per_month", value=49.5)
    )
